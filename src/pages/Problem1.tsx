  ...
  return (
    <div className="space-y-8 py-4">
      <header className="space-y-2">
        ...
        <p className="text-sm text-slate-500 font-medium">{isEnglish ? 'Application of Manning and partial filling ratios for a circular section.' : 'Aplicación de Manning y relaciones de llenado parcial para una sección circular.'}</p>
      </header>
        ...
        <section className="lg:col-span-12 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
            <h3 className="text-xs font-bold text-slate-800 uppercase tracking-widest flex items-center">
              ...
              {t.common.inputData}: {isEnglish ? 'Geometry and Slope' : 'Geometría y Pendiente'}
            </h3>
            
            <div className="space-y-5">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{isEnglish ? 'Sewer Diameter' : 'Diámetro Colector'}</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{isEnglish ? 'Slope (j)' : 'Pendiente (j)'}</label>
                <div className="relative">
                  <input 
                    type="number" 
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{isEnglish ? 'Roughness (n)' : 'Rugosidad (n)'}</label>
                <input 
                  type="number" 
                  step="0.001"
                  value={params.n || ''} 
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
            <h3 className="text-xs font-bold text-slate-800 uppercase tracking-widest flex items-center">
              <span className="w-1.5 h-1.5 bg-blue-500 rounded-full mr-2"></span>
              {t.common.inputData}: {isEnglish ? 'Population and Demand' : 'Población y Dotación'}
            </h3>
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{isEnglish ? 'Population (inh)' : 'Población (hab)'}</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{isEnglish ? 'Demand (L/inh/day)' : 'Dotación (l/h/d)'}</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{isEnglish ? 'Peak Coeff. (Cp)' : 'C. Punta (Cp)'}</label>
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
            <h3 className="text-xs font-bold text-slate-800 uppercase tracking-widest flex items-center">
              <span className="w-1.5 h-1.5 bg-blue-500 rounded-full mr-2"></span>
              {t.common.inputData}: {isEnglish ? 'Stormwater' : 'Pluviales'}
            </h3>
            <div className="space-y-4">
               <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{isEnglish ? 'Stormwater Flow (L/s)' : 'Q Pluviales (l/s)'}</label>
              ...
              <div className="p-4 bg-blue-50/50 rounded-xl border border-blue-100 flex flex-col justify-center gap-1">
                 <p className="text-[9px] font-black text-blue-900 uppercase">{isEnglish ? 'Flow Summary' : 'Resumen Caudales'}</p>
                 ...
                 <p className="text-sm font-black text-blue-900 mt-2 pt-2 border-t border-blue-200">{isEnglish ? 'TOTAL FLOW' : 'Q_TOTAL'}: {(solution.q_total_ls || 0).toFixed(1)} l/s</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="lg:col-span-12 space-y-6">
          <div className={`p-6 rounded-xl border-t-4 shadow-sm ${solution.isValid ? 'bg-white border-t-green-500' : 'bg-white border-t-orange-500'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-[10px] font-bold uppercase text-slate-500">
                    <span>{isEnglish ? 'Peak velocity (Wastewater)' : 'Velocidad punta (Residual)'}</span>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-[10px] font-bold uppercase text-slate-500">
                    <span>{isEnglish ? 'Max velocity (Stormwater)' : 'Velocidad máxima (Pluviales)'}</span>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-[10px] font-bold uppercase text-slate-500">
                    <span>{isEnglish ? 'Max Filling' : 'Llenado Máximo'}</span>
              ...
              <div className="flex items-start gap-3">
                ...
                <p className="text-[11px] font-medium text-slate-600 leading-relaxed uppercase tracking-tight">
                  {solution.isValid 
                    ...
                    : (isEnglish ? 'It is recommended to review the design parameters.' : 'Se recomienda revisar los parámetros de diseño.')}
                </p>
              </div>
            </div>
          </div>
      ...
      <ProblemSolution steps={[
        {
          title: isEnglish ? "1. Section Geometric Parameters" : "1. Parámetros Geométricos de la Sección",
          description: isEnglish 
            ? "First we calculate the properties of the pipe at full section (100% full). The area (areaFull) and the hydraulic radius (radiusHydraulicFull) depend solely on the diameter."
            : "Primero calculamos las propiedades de la tubería a sección completa (100% llena). El área (areaFull) y el radio hidráulico (radiusHydraulicFull) dependen únicamente del diámetro.",
          formula: "areaFull = π · D² / 4 | radiusHydraulicFull = D / 4",
          calcLabel: isEnglish ? "Area and Radius Calculation" : "Cálculo de Área y Radio",
        ...
        {
          title: isEnglish ? "2. Full Section Velocity (Vll)" : "2. Velocidad a Sección Llena (Vll)",
          description: isEnglish
            ? "Using Manning's formula, we determine the theoretical velocity of the water if the collector were totally full, considering its roughness and slope."
            : "Utilizando la fórmula de Manning, determinamos la velocidad teórica del agua si el colector estuviera totalmente lleno, considerando su rugosidad y pendiente.",
          formula: "v_ll = (1/n) · radiusHydraulicFull^(2/3) · J^(1/2)",
          calcLabel: isEnglish ? "Full Velocity Calculation" : "Cálculo de Velocidad Llena",
        ...
        {
          title: isEnglish ? "3. Maximum Capacity (Qll)" : "3. Capacidad Máxima (Qll)",
          description: isEnglish
            ? "By multiplying the area by the velocity, we obtain the maximum flow that the section can transport."
            : "Multiplicando el área por la velocidad, obtenemos el caudal máximo que puede transportar la sección.",
          formula: "Q_ll = areaFull · v_ll",
          calcLabel: isEnglish ? "Maximum Flow Calculation" : "Cálculo de Caudal Máximo",
          calculation: `${(solution.full?.areaFull || 0).toFixed(3)} · ${(solution.full?.vFull || 0).toFixed(2)}`,
          result: `${(solution.full?.qFull || 0).toFixed(3)} m³/s`
        },
        {
          title: isEnglish ? "4. Flow Scenarios" : "4. Escenarios de Caudal",
          description: isEnglish
            ? "We calculate the minimum, peak wastewater, and total stormwater flows to verify behavior in all loading phases."
            : "Calculamos los caudales mínimo, punta residual y total pluvial para verificar el comportamiento en todas las fases de carga.",
          calcLabel: isEnglish ? "Derived Flows" : "Caudales Derivados",
        ...
        {
          title: isEnglish ? "5. Thormann-Franke Tables Interpolation" : "5. Interpolación en Tablas de Thormann-Franke",
          description: isEnglish
            ? "With the Q/Qll ratios for each scenario, we interpolate in the tables to find the velocity and depth ratios."
            : "Con las razones Q/Qll para cada escenario, interpolamos en las tablas para hallar la relación de velocidades y calado.",
        ...
        {
          title: isEnglish ? "6. Results and Verification" : "6. Resultados y Verificación",
          description: isEnglish
            ? "Self-cleansing conditions (v_min > 0.3 m/s), peak velocity (v_p < 3 m/s), and maximum capacity (y/D < 0.8) are verified."
            : "Se verifican las condiciones de autolimpieza (v_min > 0.3 m/s), velocidad punta (v_p < 3 m/s) y capacidad máxima (y/D < 0.8).",
          formula: "v = (v/v_ll) · v_ll | y = (y/D) · D",
          calcLabel: isEnglish ? "Real Velocities Calculation" : "Cálculo de Velocidades Real",
          calculation: `v_min = ${(solution.vMin || 0).toFixed(2)} | v_p = ${(solution.vP || 0).toFixed(2)} | v_max = ${(solution.vMax || 0).toFixed(2)}`,
          result: `${(solution.vMax || 0).toFixed(2)} m/s (${isEnglish ? 'peak' : 'pico'}) | ${((solution.yOverD || 0)*params.diameter_mm).toFixed(1)} mm ${isEnglish ? 'depth' : 'calado'}`
        }
      ]} />
    </div>
